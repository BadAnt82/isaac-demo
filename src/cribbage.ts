...
export function initCribbage() {
  ...
  function connectNetwork() {
    ...
    socket.addEventListener("message", (event) => { let message: any; try { message = JSON.parse(event.data); } catch { return; }
      ...
      if (message.type === "cribbage-created" || message.type === "cribbage-joined") { state.network.gameId = message.gameId; state.network.sessionId = message.sessionId; state.network.seat = message.seat; state.network.host = Boolean(message.host); state.network.remote = true; networkStartButton.hidden = !state.network.host; cancelGameButton.hidden = !state.network.host; localStorage.setItem("badant-cribbage-network", JSON.stringify({ gameId: message.gameId, sessionId: message.sessionId })); networkStatus.textContent = state.network.host ? `Game ${message.gameId} created. Share this code, then wait for players.` : `Joined ${message.gameId}. Waiting for the host to deal.`; return; }
      ...
      if (message.type === "cribbage-cancelled") { state.network.remote = false; state.network.host = false; state.network.gameId = ""; state.network.sessionId = ""; state.network.seat = -1; networkStartButton.hidden = true; cancelGameButton.hidden = true; createGameButton.disabled = false; localStorage.removeItem("badant-cribbage-network"); networkStatus.textContent = "Game cancelled. Choose another game or create a new one."; setPanels("network"); return; }

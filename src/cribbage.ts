...
export function initCribbage() {
  ...
  function renderBoard() {
    ...
    if (state.phase === "discard") { turnLabel.textContent = `${activePlayer?.name || player.name}: choose ${needed} card${needed === 1 ? "" : "s"} to ${state.variant === "crazy" ? "pass to the dealer" : "pass to the crib"}`; status.textContent = canAct ? `${state.crib.length} crib cards placed so far.` : `${activePlayer?.name || "Opponent"} is choosing cards.`; cribAction.hidden = false; cribAction.disabled = !canAct || state.selected.size !== needed; cribAction.textContent = state.variant === "crazy" ? `Pass ${needed} cards` : `Discard ${needed} card${needed === 1 ? "" : "s"}`; passButton.hidden = true; }
